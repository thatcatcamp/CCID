...
again:
				...
				DEBUG_INFO1("Forcing interface to detact to claim...");
				if( LIBUSB_ERROR_NOT_SUPPORTED == libusb_set_auto_detach_kernel_driver(dev_handle, 1)){
					DEBUG_INFO1("Auto detach is not supported?");
				}
...
#define BSEQ_OFFSET 6
		...
		&& (buffer[BSEQ_OFFSET] != bSeq))
	{
		...
		if ((usbDevice[reader_index].workarounds &
		    ...
		    (buffer[BSEQ_OFFSET] == *usbDevice[reader_index].ccid.pbSeq-2))
			return STATUS_SUCCESS;
